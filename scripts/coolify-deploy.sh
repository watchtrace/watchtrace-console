...
Usage:
  ...
  coolify-deploy.sh deploy-image APPLICATION_UUID IMAGE_NAME sha256:DIGEST
...
case "$operation" in
  ...
  deploy-image)
    [ "$#" -eq 3 ] || usage
    uuid=$1; image=$2; digest=$3
    fail_if_invalid_uuid "$uuid"; validate_image "$image"; hex=$(digest_hex "$digest")
    desired_tag="sha256-$hex"; desired_reference="$image@sha256:$hex"
    inspect_image "$uuid" "$image"; previous_reference=$current_reference
    update_body=$(jq -cn --arg tag "$desired_tag" '{docker_registry_image_tag: $tag}')
    api_request PATCH "/applications/$uuid" "$update_body" >/dev/null || fail "Could not update the image digest for $uuid."
    trigger_and_wait "$uuid"
    deployed_tag=$(printf '%s' "$deployment_json" | jq -r '.docker_registry_image_tag // empty')
    [ "$deployed_tag" = "$desired_tag" ] || fail "Deployment $deployment_uuid reports tag '$deployed_tag', expected '$desired_tag'."
