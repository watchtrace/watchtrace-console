...
public_url=${1%/}
case "$public_url" in https://*) ;; *) echo "the public deployment URL must use HTTPS" >&2; exit 2 ;; esac
...
authority=${authority%%/*}
case "$authority" in *:*|*@*|"") echo "use an HTTPS hostname without credentials or a custom port" >&2; exit 2 ;; esac
temporary_directory=$(mktemp -d)
trap 'rm -rf "$temporary_directory"' EXIT HUP INT TERM

curl --fail --silent --show-error --location --proto '=https' --tlsv1.2 \
  --connect-timeout 10 --max-time 30 --output "$temporary_directory/health" "$public_url/health"
...
[ "$auth_status" = "401" ]
jq -e '.error.code == "invalid_session"' "$temporary_directory/auth.json" >/dev/null

redirect_status=$(curl --silent --show-error --connect-timeout 10 --max-time 30 \
  --output /dev/null --write-out '%{http_code}' "http://$authority/health")
case "$redirect_status" in 301|302|307|308) ;; *) echo "HTTP did not redirect to HTTPS" >&2; exit 1 ;; esac
...
echo "Frontend, API proxy, HTTPS redirect, and certificate checks passed."
